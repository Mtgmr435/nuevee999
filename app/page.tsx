...
// Supabase data layer
import { getUserData, saveUserData, updateStreak } from "@/lib/progress"
...
export default function Nu9veAcademy() {
  ...
  const { user, loading } = useAuth() // Supabase user | null
  ...
  // Regen de vidas (cada 15s +1 hasta max)
  useEffect(() => {
  ...
  // Cargar perfil + racha al iniciar sesión (Supabase)
  useEffect(() => {
    const run = async () => {
      if (!user) return
      ...
      if (profile) {
        setUserData((prev) => ({
          ...
          lastDailyChest: profile.last_daily_chest ?? null,
          completedLevels: profile.completed_levels ?? [],
          badges: profile.badges ?? [],
          currentPet: profile.current_pet ?? "baby-capybara",
          unlockedPets: profile.unlocked_pets ?? ["baby-capybara"],
        }))
      }

      // Actualizar/crear racha
      const s = await updateStreak(
        user.id,
        (user.user_metadata as any)?.full_name || (user as any)?.email || "Jugador Anónimo",
      )
  ...
  // Autosave a Supabase cuando cambie userData
  useEffect(() => {
    if (!user) return
    const t = setTimeout(() => {
      void saveUserData(user.id, {
        level: userData.level,
        xp: userData.xp,
        coins: userData.coins,
        gems: userData.gems,
        lives: userData.lives,
        max_lives: userData.maxLives,
        last_daily_chest: userData.lastDailyChest,
        completed_levels: userData.completedLevels,
        badges: userData.badges,
        current_pet: userData.currentPet,
  ...
  const completeLevelLocal = (levelId: number, xp: number, coins: number, badges: string[]) => {
    // sumar recompensas
    setUserData((prev) => {
      ...
      return {
        ...
        badges: badges.length ? Array.from(new Set([...(prev.badges || []), ...badges])) : prev.badges,
      }
    })

    // desbloquear siguiente en memoria (UI)
    const nextIndex = communicationLevels.findIndex((l) => l.id === levelId + 1)
    if (nextIndex !== -1) {
      communicationLevels[nextIndex].isUnlocked = true
    }
  ...
  const renderDashboard = () => (
    <div
      ...
      style={{ backgroundImage: `url('/jungle-adventure-background.jpg')` }}
    >
      <div className="absolute inset-0 bg-gradient-to-br from-amber-900/30 via-orange-900/20 to-yellow-900/30" />
        ...
        <div className="flex items-center justify-between bg-white/90 backdrop-blur-md rounded-2xl p-6 shadow-lg">
          <div className="flex items-center gap-4">
            <div className="text-5xl animate-bounce">
              {pets.find((p) => p.id === userData.currentPet)?.icon || "🐹"}
            </div>
            <div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2">
              ...
              {userData.lives < userData.maxLives && (
                <div className="text-xs text-red-600 ml-1">+1 en {Math.max(0, 15 - lifeTimer)}s</div>
              )}
            </div>
          </div>
        </div>
            ...
            <Progress
              value={
                (userData.completedLevels.length /
                  courses.reduce((acc, c) => acc + c.totalLevels, 0)) *
                100
              }
              className="h-2"
            />
          ...
          <main className="min-h-screen w-full">
            ...
            {currentView === "level" && currentLevel && (
              <LevelComponent
                ...
                userData={userData}
                onComplete={(xp, coins, badges) => completeLevelLocal(currentLevel, xp, coins, badges)}
